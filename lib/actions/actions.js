...
export const getNews = async () => {
    try {
        const response = await fetch(`${baseUrl}/wp-json/wp/v2/posts?per_page=8&_embed`, { next: { revalidate: 3600 } });
...
export const getPostsByCategory = async (categoryId, perPage = 6) => {
    try {
        const response = await fetch(
            `${baseUrl}/wp-json/wp/v2/posts?categories=${categoryId}&per_page=${perPage}&_embed`,
        ...
        const formattedData = data.map(post => ({
            ...
            content: post.content.rendered,
            excerpt: post.excerpt.rendered,
            url: post.slug, // matching your products style
            categories: post.categories,
            date: post.date,
        }));
...
export const getSingleProduct = async (slug) => {
    try {
        ...
        return {
            ...
            description: product.acf.short_description,
            sheetUrl: product.acf.sheet_url,
            right_side_video: product.acf.right_side_video,
            sheetEditionUrl: product.acf.sheet_edition_url,
            categories: product.categories,
            date: product.date,
        };
